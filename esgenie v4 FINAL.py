...
# Unconstrained optimisation per Pedersen et al. (2021)
...
[data-testid="stSidebar"] {{background:var(--soft-bg)!important;
...
def portfolio_esg_weighted(x1, x2, esg1, esg2):
    ...
    ESG score of risky positions only (risk-free asset contributes no ESG).
...
def objective(x, r1, r2, r_free, sd1, sd2, rho, gamma, theta, esg1, esg2,
    ...
    Audit-verified:
      ...
      - Corner solution at x2=0 when θ is large and esg1 >> esg2
    """
    if excluded is None:
        excluded = {}
...
def run_optimisation(r1, r2, r_free, sd1, sd2, rho,
    ...
    xrf_mv = 1.0 - x1_mv - x2_mv

    # ── Frontier: trace optimal scale at each risky-basket composition ──
    n        = 500
    w1_vals  = np.linspace(0, 1, n)
    frontier_x1, frontier_x2 = [], []
    frontier_ret, frontier_sd, frontier_esg, frontier_obj = [], [], [], []
    ...
    for w1 in w1_vals:
        ...
        s_opt       = (excess_dir / (gamma * var_dir)
                       if var_dir > 1e-12 and excess_dir > 0 else 0.0)
        fx1, fx2    = w1 * s_opt, w2 * s_opt
    ...
    def _metrics(x1, x2):
        return (portfolio_ret_2asset(x1, x2, r1, r2, r_free),
                portfolio_sd_2asset(x1, x2, sd1, sd2, rho),
                portfolio_esg_weighted(x1, x2, esg1, esg2),
                sharpe_ratio_2asset(x1, x2, r1, r2, r_free, sd1, sd2, rho))

    ret_opt, sd_opt, esg_opt, sr_opt = _metrics(x1_opt, x2_opt)
    ret_tan, sd_tan, esg_tan, sr_tan = _metrics(x1_tan, x2_tan)
    ret_mv,  sd_mv,  esg_mv,  sr_mv  = _metrics(x1_mv,  x2_mv)
    ...
    return dict(
        ...
        frontier_x1=np.array(frontier_x1), frontier_x2=np.array(frontier_x2),
...
# ══════════════════════════════════════════════════════════════════════
# HOW TO USE — original narrative restored
# ══════════════════════════════════════════════════════════════════════
...
# ══════════════════════════════════════════════════════════════════════
# INPUT SECTIONS — original narrative/tips restored
# ══════════════════════════════════════════════════════════════════════
...
with tab1:
    ...
    # Corner solution warning — check (c) from audit
    if res["is_corner"]:
        st.warning(
            f"⚠️ **Corner solution detected.** At θ = {theta}, the optimiser has set one "
            f"asset's weight to zero. This is mathematically correct — your ESG preference "
            f"is strong enough that holding the lower-ESG asset reduces utility. "
            f"Objective value at this corner: {res['obj_opt']:.6f}."
        )
    ...
    # Asset allocation — shows x1, x2, x_rf
    ...
    # Recommendation narrative — original wording restored
...
with tab2:
    ...
    st.markdown('<div class="chart-info">Each point is a portfolio at the optimal risky scale for a different asset-1 ratio, coloured by portfolio ESG score. The star marks your recommended portfolio. Hover for details.</div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="chart-info">Shows how the objective value changes as the share of Asset 1 in the risky basket varies, each evaluated at its optimal total scale. The star marks the maximum — your recommended allocation.</div>', unsafe_allow_html=True)
...
with tab3:
    ...
    st.markdown('<div class="chart-info">ℹ️ How to read this: higher θ puts more weight on ESG; higher γ reduces total risky exposure — both x₁ and x₂ fall proportionally. The heatmap shows how portfolio ESG score responds across all combinations of θ and γ — your profile is marked with a star.</div>', unsafe_allow_html=True)
